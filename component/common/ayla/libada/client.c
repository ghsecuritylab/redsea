...
static void client_err_cb(struct http_client *hc)
{
	...
	case HC_ERR_HTTP_STATUS:
		/*
		 * If authentication failed, clear out the auth key and
		 * the client_key to force re-authentication.
		 * Also, fix the time if it's not close.
		 */
		if (hc->http_state.status == HTTP_STATUS_UNAUTH &&
		    (state->tgt != CCT_IMAGE_SERVER || !state->ota.auth_fail)) {
			...
			if (hc->server_time) {
				...
				state->auth_fails = CLIENT_AUTH_FAILS;
				goto auth_error;
			}
			hc->auth_hdr[0] = '\0';
			goto next_step;
