...
int MQTTStringFormat_connect(char* strbuf, int strbuflen, MQTTPacket_connectData* data)
{
	...
	strindex = rtl_snprintf(strbuf, strbuflen,
			"CONNECT MQTT version %d, client id %.*s, clean session %d, keep alive %d",
			(int)data->MQTTVersion, data->clientID.lenstring.len, data->clientID.lenstring.data,
			(int)data->cleansession, data->keepAliveInterval);
	if (data->willFlag)
		strindex += rtl_snprintf(&strbuf[strindex], strbuflen - strindex,
...
{
	int strindex = rtl_snprintf(strbuf, strbuflen, "CONNACK session present %d, rc %d", sessionPresent, connack_rc);
	return strindex;
}
...
{
	int strindex = rtl_snprintf(strbuf, strbuflen,
...
{
	int strindex = rtl_snprintf(strbuf, strbuflen, "%s, packet id %d", MQTTPacket_names[packettype], packetid);
	if (dup)
		strindex += rtl_snprintf(strbuf + strindex, strbuflen - strindex, ", dup %d", dup);
	return strindex;
}
...
{
	return rtl_snprintf(strbuf, strbuflen,
...
char* MQTTFormat_toClientString(char* strbuf, int strbuflen, unsigned char* buf, int buflen)
{
	...
	case PINGRESP:
	case DISCONNECT:
		rtl_snprintf(strbuf, strbuflen, "%s", MQTTPacket_names[header.bits.type]);
		break;
	}
	return strbuf;
}
